...
TEST(TxnBonusTest, SerializableTest) {  // NOLINT
  fmt::println(stderr, "--- SerializableTest2: Serializable ---");
  {
    ...
    TxnMgrDbg("after select", bustub->txn_manager_.get(), table_info, table_info->table_.get());
    ...
    TxnMgrDbg("after commit txn3", bustub->txn_manager_.get(), table_info, table_info->table_.get());
    WithTxn(txn_read, CommitTxn(*bustub, _var, _txn));
    TxnMgrDbg("after commit txn_read", bustub->txn_manager_.get(), table_info, table_info->table_.get());
