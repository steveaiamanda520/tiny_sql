...
TEST(TxnIndexTest, InsertDeleteTest) {  // NOLINT
  ...
  auto txn3 = BeginTxn(*bustub, "txn3");
  WithTxn(txn3, ExecuteTxn(*bustub, _var, _txn, "INSERT INTO maintable VALUES (1, 1), (2, 1), (3, 1), (5, 1)"));
  WithTxn(txn3, QueryShowResult(*bustub, _var, _txn, query, IntResult{{1, 1}, {2, 1}, {3, 1}, {5, 1}}));
  WithTxn(txn3, QueryIndex(*bustub, _var, _txn, query, "col1", std::vector<int>{1, 2, 3, 4, 5},
                           IntResult{{1, 1}, {2, 1}, {3, 1}, {}, {5, 1}}));
  WithTxn(txn3, CommitTxn(*bustub, _var, _txn));
  TxnMgrDbg("after txn3 insert", bustub->txn_manager_.get(), table_info, table_info->table_.get());

  TableHeapEntryNoMoreThan(*bustub, table_info, 5);

  auto txn3_reverify = BeginTxn(*bustub, "txn3_reverify");
  auto txn4 = BeginTxn(*bustub, "txn4");
  WithTxn(txn3_reverify, QueryShowResult(*bustub, _var, _txn, query, IntResult{{1, 1}, {2, 1}, {3, 1}, {5, 1}}));
  WithTxn(txn4, ExecuteTxn(*bustub, _var, _txn, "DELETE FROM maintable"));
  WithTxn(txn4, QueryShowResult(*bustub, _var, _txn, query, IntResult{}));
  WithTxn(txn4, CommitTxn(*bustub, _var, _txn));
  TxnMgrDbg("after txn4 delete", bustub->txn_manager_.get(), table_info, table_info->table_.get());

  auto txn5 = BeginTxn(*bustub, "txn5");
  WithTxn(txn5, ExecuteTxn(*bustub, _var, _txn, "INSERT INTO maintable VALUES (1, 2), (2, 2), (3, 2), (4, 2)"));
  WithTxn(txn5, QueryShowResult(*bustub, _var, _txn, query, IntResult{{1, 2}, {2, 2}, {3, 2}, {4, 2}}));
  WithTxn(txn5, QueryIndex(*bustub, _var, _txn, query, "col1", std::vector<int>{1, 2, 3, 4, 5},
                           IntResult{{1, 2}, {2, 2}, {3, 2}, {4, 2}, {}}));
  ...
  WithTxn(txn1_reverify, QueryIndex(*bustub, _var, _txn, query, "col1", std::vector<int>{1, 2, 3, 4, 5},
                                    IntResult{{1, 0}, {2, 0}, {3, 0}, {4, 0}, {}}));
  WithTxn(txn3_reverify, QueryShowResult(*bustub, _var, _txn, query, IntResult{{1, 1}, {2, 1}, {3, 1}, {5, 1}}));
